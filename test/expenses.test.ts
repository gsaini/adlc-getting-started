...
describe("List expenses", () => {
	it("Newest first", async () => {
		const g = await createGroup();
		await addExpense(g.id, { payer: "Asha", amountCents: 1200, description: "Taxi" });
		await addExpense(g.id, { payer: "Ben", amountCents: 3000, description: "Museum" });
		const res = await api("GET", `/groups/${g.id}/expenses`);
		expect(res.status).toBe(200);
		const expenses = res.json?.expenses as { description: string; shares: unknown[] }[];
		expect(expenses.map((e) => e.description)).toEqual(["Museum", "Taxi"]);
		expect(expenses[0]).toMatchObject({ payer: "Ben", amountCents: 3000 });
		expect(expenses[0]?.shares).toHaveLength(3);
	});
	...
	it("No expenses yet", async () => {
		...
		const res = await api("GET", `/groups/${g.id}/expenses`);
		expect(res.status).toBe(200);
		expect(res.json).toEqual({ expenses: [] });
	});
